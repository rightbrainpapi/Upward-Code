...
function addPopupListeners() {
	...
	var imageLinks = $(
		'a[href$=".png"], a[href$=".jpg"], a[href$=".gif"], a[href$=".bmp"]'
	);
	...
	if (imageLinks.children("img").length) {
		...
		imageLinks.click(function(e) {
			...
			$("#overlay-wrapper img").attr(
				"src",
				"https://raw.github.com/Snacksnack/snacksnack.github.com/master/template/" +
					$(this).context.pathname.split("/pen/")[1]
			);
			$("#overlay-wrapper").fadeIn(300);
			$("#overlay-wrapper img").attr("title", "Click anywhere to close");
			$("#overlay-wrapper img").css({
				"margin-left":
					($(window)[0].innerWidth - $("#overlay-wrapper img").width()) / 2,
				"margin-top":
					($(window)[0].innerHeight - $("#overlay-wrapper img").height()) / 2
			});
		});
	}
